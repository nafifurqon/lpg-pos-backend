...
- [x] `POST /shops` requires valid access token — `401` if missing or invalid
- [x] Validates `name` (required, min 5, max 100) — `400` on violation with `messages[]`
- [x] Validates `address` (required, min 5, max 500) — `400` on violation with `messages[]`
- [x] Validates `registrationNumber` (optional, max 50) — `400` on violation if provided
- [x] Returns `201` with `{ message: "Toko berhasil didaftarkan", result: { id, name, address, registrationNumber, createdAt, updatedAt } }` on success
- [x] Returns `409` if the authenticated owner already has a registered shop
- [x] `owner_id` in `shops` table has a UNIQUE constraint enforcing one shop per owner
