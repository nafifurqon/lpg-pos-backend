...
- [x] `npm run migration:status` shows all migrations applied
- [x] `POST /shops` without token returns `401`
- [x] `POST /shops` with valid token + valid body returns `201 { message, result }`
- [x] `POST /shops` second time same owner returns `409`
- [x] `POST /shops` with `name` < 5 chars returns `400` with `messages[]`
